...
def CMDHandler(socket, addr):
    ...
    CLI_Control.set_Title(socket, f"Cronical [-] Batman Edition [-] User: {Strings.CurrentUser} [-] Online Users: {len(ServerConfig.clients)}")
    ...
    if data != "":
        ...
        if data.lower() == "help" or data.lower() == "?":
        ...
        elif data.lower() == "cronical":
            socket.send(str(Strings.MainColors['Clear'] + CustomBannerMaker.CreateMOTD(utils.GetMOTD()) + BannerModify.GetBannerFromFile("main") + BannerModify.GetBannerFromFile("cronical")).encode())
        elif data.lower() == "tools":
            socket.send(str(Strings.MainColors['Clear'] + CustomBannerMaker.CreateMOTD(utils.GetMOTD()) + BannerModify.GetBannerFromFile("main") + BannerModify.GetBannerFromFile("tools")).encode())
        elif data.lower() == "demonic":
            socket.send(str(Strings.MainColors['Clear'] + CustomBannerMaker.CreateMOTD(utils.GetMOTD()) + BannerModify.GetBannerFromFile("main") + BannerModify.GetBannerFromFile("demonic")).encode())
        elif "geo" in data:
            ...
            socket.send(f"Invalid Command".encode())

        socket.send(str(f"\x1b[37m╔═[\x1b[33m{Strings.CurrentUser}\x1b[33m@\x1b[33mcronical\x1b[37m]\r\n╚════➢\x1b[323m ").encode())
