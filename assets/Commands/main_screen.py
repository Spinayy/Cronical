...
def login(socket, addr):
    if NetSettings.login == True:
        ...
        if "[+]" in Auth.Login(username, password, addr[0]): # This is a weird way of authentication lol 
            ...
            socket.send(f"Welcome to Cronical {username}\r\n\x1b[37m╔═[\x1b[33m{Strings.CurrentUser}\x1b[33m@\x1b[33mcronical\x1b[37m]\r\n╚════➢\x1b[33m ".encode())
            ...
            socket.send(f"{Strings.MainColors['Reset']}Incorrect username or password. Try again....".encode())
            time.sleep(4)
            socket.close()
    else:
        socket.send("[x] Error, Cronical NET login is currently disabled for Updates, try again later....\r\n".encode())
        socket.close()
